...
export default function AutoMod({ token, guild, apiFetch, user }) {
  ...
  // Auto-save config to API whenever it changes (debounced 600ms)
  ...
  useEffect(() => {
    ...
    saveTimeoutRef.current = setTimeout(() => {
      apiFetch(`/guild/${guild.id}/automod`, token, { method: "POST", body: JSON.stringify(config) });
    }, 600);
  ...
  function addBypassRole(roleId) {
    if (!roleId) return;
    const current = config.bypassRoles || [];
    if (current.includes(roleId)) return;
    setConfig(c => ({ ...c, bypassRoles: [...current, roleId] }));
  }

  function removeBypassRole(roleId) {
    setConfig(c => ({ ...c, bypassRoles: (c.bypassRoles || []).filter(id => id !== roleId) }));
  }
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      {config.enabled && (<>
        ...
        {/* Bypass Roles */}
        <Card>
          <SectionTitle>🛡️ Bypass Roles</SectionTitle>
          <div style={{ color: TEXT_DIM, fontSize: 13, marginBottom: 14 }}>
            Members with any of these roles will be completely exempt from Auto Mod.
          </div>
          <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
            <select
              defaultValue=""
              onChange={e => { addBypassRole(e.target.value); e.target.value = ""; }}
            ...
            >
              <option value="">+ Add bypass role...</option>
              {allRoles.filter(r => !bypassRoles.includes(r.id)).map(r => (
                <option key={r.id} value={r.id}>@{r.name}</option>
              ))}
            </select>
          </div>
          {bypassRoles.length === 0 ? (
            <div style={{ color: TEXT_DIM, fontSize: 13 }}>No bypass roles set — Auto Mod applies to everyone (except admins & server owner).</div>
          ) : (
            <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              {bypassRoles.map(id => {
                const role = allRoles.find(r => r.id === id);
                return (
                  <div key={id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", background: SURFACE2, border: `1px solid ${BORDER}`, borderRadius: 7, padding: "8px 12px" }}>
                    <span style={{ color: TEXT, fontSize: 13, fontWeight: 600 }}>@{role?.name ?? id}</span>
                    <button onClick={() => removeBypassRole(id)} style={{ background: RED + "22", border: `1px solid ${RED}44`, color: RED, borderRadius: 6, padding: "4px 10px", cursor: "pointer", fontSize: 12, fontWeight: 700 }}>Remove</button>
                  </div>
                );
              })}
            </div>
